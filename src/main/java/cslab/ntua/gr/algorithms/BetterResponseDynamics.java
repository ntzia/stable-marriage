...
public class BetterResponseDynamics extends Abstract_SM_Algorithm
{
    private int[][] married;

    public BetterResponseDynamics(int n, String menFileName, String womenFileName)
    {
    	super(n, menFileName, womenFileName);
    }

    // Constructor for when agents are available
    public BetterResponseDynamics(int n, Agent[][] agents)
    {
        super(n, agents);
    }
    ...
    public Marriage match()
    {
        ...
        while (true)
        {
            found_bp = false;
            // Decide side
            side = ThreadLocalRandom.current().nextInt(0, 2);
            other_side = flip(side);
            // Randomize order of agents to check for blocking pairs
            java.util.Collections.shuffle(proposers);
            for (int p : proposers)
            {
                // Randomize order of indices to check for blocking pairs
    ...
    public static void main(String args[]) 
    {
        ...
        Abstract_SM_Algorithm smp = new BetterResponseDynamics(n, menFile, womenFile);
