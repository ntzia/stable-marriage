...
public class GS_MaleOpt extends Abstract_SM_Algorithm
{
    ...
    private boolean terminate()
    {
    	for (int i = 0; i < n; i++)
    	{
    		if (mIndex[0][i] == Integer.MAX_VALUE) return false;
    	}
		return true;
    }
    ...
    {   
        ...
        if (mIndex[0][proposer] == Integer.MAX_VALUE)
        {
            // Wants to propose
            ...
            {
                //Engage with new
                mIndex[0][proposer] = proposeToIndex;
            }
            else
            {
                // rejected
    ...
    private boolean evaluate(int acceptor, int proposer)
    {
        ...
        int marriedToIndex = mIndex[1][acceptor];
        if (marriedToIndex > proposerRank)
        {
            ...
            if (marriedToIndex != Integer.MAX_VALUE)
            {
                ...
                mIndex[0][old] = Integer.MAX_VALUE;                
            }
            
            //Engage with new
            mIndex[1][acceptor] = proposerRank;
