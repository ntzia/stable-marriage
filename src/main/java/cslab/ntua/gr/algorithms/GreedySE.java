...
public class GreedySE extends Abstract_SM_Algorithm
{
    private int[][] kappa, lambda;
    private boolean married[][];
    private int curr_side_cost[];
    ...
    public GreedySE(int n, String menFileName, String womenFileName)
    {
    ...
    public int[] match()
    {
        ...
        while (candidate_proposers.size() != 0)
        {
            ...
            curr_cost = m.cost;
            curr_side_cost[0] = m.men_cost;
            curr_side_cost[1] = m.women_cost;
            ...
            for (idx = kappa[side][p]; idx <= lambda[side][p]; idx++)
            {
                ...
                if (agents[flip(side)][r].getRankOf(p) <= lambda[flip(side)][r])
                {
                    ...
                    if (married[flip(side)][r])
                    {
                        ...
                        new_side_cost[side] = curr_side_cost[side] - agents[side][previous_spouse].getRankOf(r) + agents[side][p].getRankOf(r);
                        new_side_cost[flip(side)] = curr_side_cost[flip(side)] - agents[flip(side)][r].getRankOf(previous_spouse) + agents[flip(side)][r].getRankOf(p);
                        c = (singles / 2) * n + Math.abs(new_side_cost[0] - new_side_cost[1]);
                    }
                    else
                    {
                        new_side_cost[side] = curr_side_cost[side] + agents[side][p].getRankOf(r);
                        new_side_cost[flip(side)] = curr_side_cost[flip(side)] + agents[flip(side)][r].getRankOf(p);
                        c = ((singles - 2) / 2) * n + Math.abs(new_side_cost[0] - new_side_cost[1]);
                    }
                    ...
                    m = new Move(p, side, r, c, new_side_cost[0], new_side_cost[1], times_executed, associated_rem);
    ...
    public static void main(String args[]) 
    {
        ...
        Abstract_SM_Algorithm smp = new GreedySE(n, menFile, womenFile);
...
*/        
    ...
    { 
        ...
        public int cost, men_cost, women_cost;
        public int times_executed;
        public Reminiscence associated_reminescence;

        public Move(int p, int proposing_side, int r, int c, int mc, int wc, int times_ex, Reminiscence rem) 
        { 
            this.proposer = p; 
